  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-50 border-b border-stone-200/80 bg-white/90 backdrop-blur-xl">
        ...
        <div className="bg-primary-950 text-white text-[0.65rem] py-1.5 hidden lg:block overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            <label className="inline-flex items-center gap-1 rounded-full bg-white px-2.5 py-1 shadow-sm shrink-0">
              <Globe className="h-3 w-3 shrink-0 text-primary-700" aria-hidden />
              <span className="sr-only">Language</span>
              <select
                value={language}
                onChange={(e) => setLanguage(parseLanguage(e.target.value))}
                aria-label="Select language"
                className="cursor-pointer bg-transparent pl-1 pr-5 text-[11px] font-semibold text-stone-900 focus:outline-none"
              >
            ...
            <nav className="px-3 pb-3 space-y-0.5">
              ...
              <div className="px-3 py-2">
                <label className="flex items-center gap-3 rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5">
                  <Globe className="h-4 w-4 shrink-0 text-primary-700" aria-hidden />
                  <span className="text-xs font-semibold text-stone-900">Language</span>
                  <select
                    value={language}
                    onChange={(e) => setLanguage(parseLanguage(e.target.value))}
                    aria-label="Select language"
                    className="ml-auto min-w-0 flex-1 max-w-[9rem] cursor-pointer rounded-lg border border-stone-300 bg-white py-1.5 pl-2 pr-7 text-xs font-medium text-stone-900 shadow-sm focus:border-primary-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30"
                  >
                    <option value="en">English</option>
                    <option value="ha">Hausa</option>
                    <option value="yo">Yoruba</option>
